(() => {
  ...
  [...adminHead.querySelectorAll('button')].forEach(candidate => {
    if (candidate !== btn && /push pictures/i.test(candidate.textContent || '')) candidate.remove();
  });
  ...
  document.querySelectorAll('#pictureSyncStatus, .picture-sync-status').forEach((node,index) => { if(index) node.remove(); });
  let status=document.getElementById('pictureSyncStatus');
  if(!status){
    status=document.createElement('p');
    status.id='pictureSyncStatus';
    status.className='muted picture-sync-status';
  ...
  async function uploadDataUrl(entryId,dataUrl){
    ...
    const safeId=String(entryId).replace(/[^a-zA-Z0-9_-]/g,'-');
    const objectName=`${safeId}-${Date.now()}-${Math.random().toString(36).slice(2,8)}.jpg`;
    ...
    return `${config.SUPABASE_URL}/storage/v1/object/public/entry-images/${encodeURIComponent(objectName)}`;
  }
  ...
  async function pushPictures(){
    ...
    status.textContent=`Checking ${entries.length} pictures…`;
    ...
    for(const [entryId,value] of entries){
      try{
        ...
        console.error('Picture sync failed',entryId,err);
        failed.push(`${entryId}: ${err.message}`);
      }
    }
    ...
    if(failed.length){
      status.textContent=`${linked} pictures shared; ${failed.length} failed.`;
      alert(`Picture upload failed:\n\n${failed.join('\n')}`);
    } else {
      status.textContent=`${linked} pictures are shared in Supabase (${uploaded} uploaded from this iPhone).`;
    }
