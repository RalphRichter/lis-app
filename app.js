...
const seedItems = [
  { id:'fixed-arrival-ralph', date:'2026-09-08', type:'fixed', title:'Ralph · STR → ZRH → LIS', category:'Travel', time:'19:21 → 23:34', description:'SWISS via Zurich. Late arrival in Lisbon.', location:'Lisbon Airport' },
  { id:'fixed-arrival-stani', date:'2026-09-08', type:'fixed', title:'Stani · ZRH → GVA → LIS', category:'Travel', time:'09:18 → 14:15', description:'SWISS via Geneva. Meet in Lisbon.', location:'Lisbon' },
  ...
  { id:'fixed-work-wed', date:'2026-09-09', type:'fixed', title:'Ralph · HotelOffice', category:'Work', time:'08:00–17:00', description:'09:00–18:00 CEST.', location:'Hyatt Regency Lisbon' },
  ...
  { id:'fixed-work-thu', date:'2026-09-10', type:'fixed', title:'Ralph · HotelOffice', category:'Work', time:'08:00–17:00', description:'09:00–18:00 CEST.', location:'Hyatt Regency Lisbon' },
  ...
  { id:'fixed-work-fri', date:'2026-09-11', type:'fixed', title:'Ralph · HotelOffice', category:'Work', time:'08:00–17:00', description:'09:00–18:00 CEST.', location:'Hyatt Regency Lisbon' },
...
function localData() {
  return JSON.parse(localStorage.getItem('lis-data') || 'null');
}
function saveLocal() {
  localStorage.setItem('lis-data', JSON.stringify({ items: state.items, votes: state.votes }));
}
...
async function loadData() {
  if (HAS_SUPABASE) {
    try {
      ...
      state.items = [...seedItems.filter(x=>x.type==='fixed'), ...seedItems.filter(x=>x.type==='idea'), ...ideas.map(i => ({...i,type:'idea'}))];
      state.votes = votes || [];
      return;
    } catch (err) {
...
async function toggleVote(item) {
  ...
  if (HAS_SUPABASE) {
    try {
      if (existing) {
        await supabase(`votes?idea_id=eq.${encodeURIComponent(item.id)}&user_name=eq.${encodeURIComponent(state.name)}`, { method:'DELETE', headers:{Prefer:'return=minimal'} });
      } else {
        await supabase('votes', { method:'POST', headers:{Prefer:'return=representation'}, body:JSON.stringify({idea_id:item.id,user_name:state.name}) });
      }
      await loadData();
...
function render() {
  ...
  for (const day of TRIP_DAYS) {
    let items = state.items.filter(i => i.date === day.date);
    if (filter === 'fixed') items = items.filter(i => i.type === 'fixed');
    if (filter === 'idea' || filter === 'liked') items = items.filter(i => i.type === 'idea');
    if (filter === 'liked') items.sort((a,b) => voteCount(b.id) - voteCount(a.id));
    if (!items.length) continue;

    const node = document.getElementById('dayTemplate').content.cloneNode(true);
    node.querySelector('.day-date').textContent = `${day.label} · ${new Date(day.date+'T12:00:00').toLocaleDateString('en-GB',{day:'numeric',month:'long'})}`;
    node.querySelector('.day-title').textContent = day.title;
    node.querySelector('.count-pill').textContent = `${items.filter(i=>i.type==='idea').length} ideas`;
    ...
    for (const item of items) {
      ...
      iNode.querySelector('.category-chip').textContent = item.category || 'Idea';
      const t = iNode.querySelector('.time-chip');
      t.textContent = item.time || 'Flexible';
      ...
      if (item.location) meta.insertAdjacentHTML('beforeend', `<span>📍 ${escapeHtml(item.location)}</span>`);
      ...
      if (item.url) meta.insertAdjacentHTML('beforeend', `<a href="${escapeAttr(item.url)}" target="_blank" rel="noopener">Open link ↗</a>`);
...
document.getElementById('profileForm').addEventListener('submit', (e) => {
  ...
  state.name = value;
  localStorage.setItem('lis-name', value);
...
TRIP_DAYS.forEach(d => {
  const opt = document.createElement('option'); opt.value=d.date; opt.textContent=`${d.label}, ${new Date(d.date+'T12:00:00').toLocaleDateString('en-GB',{day:'numeric',month:'short'})}`; ideaDay.appendChild(opt);
});
...
document.getElementById('ideaForm').addEventListener('submit', async (e) => {
  ...
  const item = {
    id: crypto.randomUUID(), type:'idea', title,
    date: ideaDay.value,
  ...
  if (HAS_SUPABASE) {
    try {
      await supabase('ideas', { method:'POST', headers:{Prefer:'return=minimal'}, body:JSON.stringify(item) });
      await loadData();
