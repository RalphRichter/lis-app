(() => {
  ...
  function getImageValue(field) {
    return field?.dataset.localImage || field?.value.trim() || '';
  }

  function setImageValue(field, value) {
    if (!field) return;
    if (value && value.startsWith('data:image/')) {
      field.dataset.localImage = value;
      field.value = '';
    } else {
  ...
  function updatePickerPreview(field) {
    const picker = field?.closest('.image-picker');
    if (!picker) return;
    const preview = picker.querySelector('.image-picker-preview');
    const status = picker.querySelector('.image-picker-status');
    const value = getImageValue(field);
    if (!value) {
      preview.hidden = true;
      preview.removeAttribute('src');
      status.textContent = 'Optional · choose a photo, paste an image, or enter a URL.';
      return;
    }
    preview.hidden = false;
    preview.src = value;
    status.textContent = field.dataset.localImage ? 'Photo selected from this device.' : 'Image URL selected.';
  }

  async function acceptImageFile(field, file) {
    try {
      const data = await imageFileToDataUrl(file);
      field.dataset.localImage = data;
      field.value = '';
      updatePickerPreview(field);
    } catch (err) {
      alert(err.message || 'Could not read the image.');
    }
  }
      ...
      try {
        ...
        for (const item of items) {
          ...
          if (type) {
            ...
            await acceptImageFile(field, new File([blob], 'clipboard-image', {type}));
            return;
          }
        }
  ...
  function setupImagePicker(field) {
    if (!field || field.dataset.pickerReady) return;
  ...
  setupImagePicker(document.getElementById('editImage'));
  ...
  document.getElementById('ideaForm')?.addEventListener('submit', () => {
    const field = document.getElementById('ideaImage');
    const image = getImageValue(field);
    ...
    setTimeout(() => {
      ...
      if (!item) return;
      const map = imageMap(); map[item.id] = image; saveMap(map);
