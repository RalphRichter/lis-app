...
-- Votes may already exist from an older version, so create/upgrade every expected column separately.
...
-- Keep duplicate protection without requiring existing legacy rows to be non-null.
create unique index if not exists votes_idea_user_name_unique
  on public.votes (idea_id,user_name)
  where idea_id is not null and user_name is not null;
create unique index if not exists votes_idea_user_id_unique
  on public.votes (idea_id,user_id)
  where idea_id is not null and user_id is not null;
